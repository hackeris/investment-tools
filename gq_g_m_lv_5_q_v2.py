...
def _main():
    ...
    total_revenue_q = get_total_revenue_q_of_stocks(csi300)

    revenue_growth_1 = dict([(code, total_revenue_q.get(code)[0]['growth_l4q']) for code in csi300])
    revenue_growth_2 = dict([(code, total_revenue_q.get(code)[1]['growth_l4q']) for code in csi300])
    revenue_growth_3 = dict([(code, total_revenue_q.get(code)[2]['growth_l4q']) for code in csi300])
